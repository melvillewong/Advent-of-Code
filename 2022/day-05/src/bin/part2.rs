...
fn parse_and_move_crate(vec: &mut [VecDeque<char>], line: &str) {
    ...
    let mut drained: VecDeque<char> = vec[arr[1] as usize - 1].drain(..arr[0] as usize).collect();
    drained.append(&mut vec[arr[2] as usize - 1]);
    vec[arr[2] as usize - 1].clear();
    vec[arr[2] as usize - 1].append(&mut drained);
}
...
move 2 from 2 to 1
move 1 from 1 to 2";
        assert_eq!(find_top_crates(input), "MCD");
    }
}
